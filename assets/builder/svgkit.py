...
def led(x, y, color=None, r=4, blink=True):
    ...
    return (
        '<rect x="%s" y="%s" width="8" height="8" fill="%s"%s '
...
def corners(x, y, w, h, col=BCH, s=12, inset=5):
    ...
    return '<g stroke="%s" stroke-width="1" fill="none">%s</g>' % (col, "".join(p))
...
def svg(w, h, extra_css, body, bg_on=True):
    doc = (
        ...
        % (w, h, w, h, defs(), BASE_CSS, extra_css, bg(w, h) if bg_on else "", body)
    )
    return doc
...
def panel_frame(h, title, ref, y0=0):
    ...
    parts = []
    parts.append(
        '<rect x="1" y="%s" width="%s" height="%s" fill="%s" '
        'stroke="%s"/>' % (y0 + 1, W - 2, h - 2, C["black"], BC)
    )
    # label bar
    parts.append(
        '<rect x="1" y="%s" width="%s" height="26" '
        'fill="rgba(85,255,255,.05)"/>' % (y0 + 1, W - 2)
    )
    parts.append(
        '<line x1="1" y1="%s" x2="%s" y2="%s" stroke="%s"/>'
        % (y0 + 27, W - 1, y0 + 27, BC2)
    )
    parts.append(led(16, y0 + 14))
    parts.append(
        '<text x="30" y="%s" font-size="9.5" letter-spacing="1.6" '
        'fill="%s">%s</text>' % (y0 + 17.5, C["cyan"], esc(title))
    )
    parts.append(
        '<text x="%s" y="%s" font-size="9.5" letter-spacing="1.6" '
