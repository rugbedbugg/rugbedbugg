...
def build_header():
    ...
    # status bar
    b.append(
        '<text x="8" y="14" font-size="10" letter-spacing="1.5" fill="%s">'
        'NODE//rugbedbugg  ·  SIG:<tspan fill="%s">STRONG</tspan>'
        "  ·  198X MODE</text>" % (C["dim"], C["green"])
    )
    b.append(led(W - 116, 10))
    b.append(
        '<text x="%s" y="14" font-size="10" letter-spacing="1.5" '
        'text-anchor="end" fill="%s">REC 00:37:12</text>' % (W - 8, C["red"])
    )
    b.append('<line x1="8" y1="24" x2="%s" y2="24" stroke="%s"/>' % (W - 8, BC2))

    cx = W / 2
    b.append(
        '<text x="%s" y="52" font-size="10.5" letter-spacing="2.4" '
        'text-anchor="middle" fill="%s" opacity=".8">'
        "[ SYSTEM :: OXIDE TERMINAL PROFILE :: 198X MODE ]</text>" % (cx, C["cyan"])
    )
    # namemark with chromatic split
    nm = "&gt; Oxide 1-6"
    for dx, col, op in ((-2, C["red"], 0.5), (2, C["cyan"], 0.55)):
        b.append(
            '<text class="nm" x="%s" y="108" font-size="54" '
            'letter-spacing="6" text-anchor="middle" fill="%s" '
            'opacity="%s">%s</text>' % (cx + dx, col, op, nm)
        )
    b.append(
        '<text class="nm" x="%s" y="108" font-size="54" letter-spacing="6" '
        'text-anchor="middle" fill="%s" filter="url(#glow)">'
        '<tspan fill="%s">&gt;</tspan> Oxide 1-6</text>' % (cx, C["bright"], C["dim"])
    )
    b.append(
        '<text x="%s" y="130" font-size="11" letter-spacing=".9" '
        'text-anchor="middle" fill="%s">'
        "[ Oxide 1-6 // Arsenic 1-6 // rugbedbugg ]</text>" % (cx, C["dim"])
    )
    b.append(
        '<text x="%s" y="152" font-size="13" letter-spacing="2.6" '
    ...
    maxc = int(bw / (fs * 0.72))
    b.append(
        '<rect x="%s" y="%s" width="2" height="%s" fill="%s"/>'
        % (bx, fy + 2, fh - 40, BC)
    )
    tx = bx + 14
    y = fy + 14
    ...
    y += 16
    b.append(
        '<text x="%s" y="%s" font-size="%s" fill="%s">Welcome to my '
        "GitHub!</text>" % (tx, y, fs, C["text"])
    )
    y += 22
    quote = (
        "“Once I told the computer to do something and it did it "
    ...
    for i, col in enumerate(bar_cols):
        b.append(
            '<rect x="%s" y="%s" width="%s" height="4" fill="%s"/>'
            % (fx + i * sw, fy, sw + 1, col)
        )
    b.append(
        '<clipPath id="hfclip"><rect x="%s" y="%s" width="%s" height="%s"/>'
    ...
    b.append(
        '<rect x="%s" y="%s" width="%s" height="%s" fill="url(#scan)" '
        'opacity=".4"/>' % (fx, iy, fw, ih)
    )
    b.append(
        '<rect class="track" x="%s" y="%s" width="%s" height="12" '
        'fill="#fff" opacity=".10"/>' % (fx, iy, fw)
    )
    b.append(
        '<rect x="%s" y="%s" width="%s" height="%s" fill="url(#vig)"/>'
        % (fx, iy, fw, ih)
    )
    b.append("</g>")
    b.append(
        '<rect x="%s" y="%s" width="%s" height="%s" fill="none" '
        'stroke="rgba(85,255,255,.4)" stroke-width="2"/>' % (fx, fy, fw, fh)
    )
    # top OSD strip
    b.append(
        '<rect x="%s" y="%s" width="%s" height="17" fill="rgba(0,0,0,.55)"/>'
        % (fx, iy, fw)
    )
    b.append(led(fx + 9, iy + 8.5))
    b.append(
        '<text x="%s" y="%s" font-size="8" letter-spacing="1.2" fill="%s">'
        "CH 03 · CAM-01</text>" % (fx + 19, iy + 12, C["cyan"])
    )
    b.append(
        '<text x="%s" y="%s" font-size="8" letter-spacing="1.2" '
        'text-anchor="end" fill="%s">LIVE</text>' % (fx + fw - 8, iy + 12, C["red"])
    )
    # bottom OSD
    b.append(
        '<rect x="%s" y="%s" width="%s" height="34" fill="url(#nmfade)"/>'
        % (fx, iy + ih - 34, fw)
    )
    b.append(
        '<text x="%s" y="%s" font-size="8" letter-spacing=".8" fill="%s">'
